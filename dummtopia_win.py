...
def _detect_system_locale():
    try:
        lang = _locale_mod.getdefaultlocale()[0]
        if lang:
...
def getch_timeout(timeout=1.0):
    """Read a single char with timeout. Returns char or None. Cross-platform."""
    if IS_WINDOWS:
...
SAVE_FILE = os.path.expanduser("~/.dummtopia_save.json")
...
def get_menu_input(state):
    """Returns a single action string. Cross-platform."""
    if IS_WINDOWS:
        ch = msvcrt.getwch()
    else:
        fd = sys.stdin.fileno()
...
_PRE_POLICE_BLACKOUT  = 20
_POST_POLICE_BLACKOUT = 10
...
def _mafia_repay_partial(state):
    ...
    if new_debt <= 0:
        ...
        print(green(f"  ✓ €{pay} bezahlt. Verbleibende Schuld: {red(chr(8364)+str(new_debt))}"))
